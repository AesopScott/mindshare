...
Canonical source: `C:\Users\scott\Code\mindshare\roles\recruiter\Autonomy.md`
...
Level 4 logic owner: `C:\Users\scott\Code\mindshare\roles\recruiter\scripts\level4automation.py`.
...
This means Ana may take valid items from `C:\Users\scott\Code\mindshare\roles\recruiter\recruiting.backlog.md` and carry them through the internal role-hiring workflow until they reach Level 3 Staff, using timed progression rules and only the approved internal artifacts.
...
- Ana checks `C:\Users\scott\Code\mindshare\roles\recruiter\recruiting.pipeline.json` every 4 hours through the same automation for Level 1 to Level 2 and Level 2 to Level 3 promotion eligibility.
- `ana-l4-recruiting-backlog-processing` must call `C:\Users\scott\Code\mindshare\roles\recruiter\scripts\level4automation.py --write --mode scheduled` before doing packet-writing or mirror work. The script owns deterministic backlog counts, timed-promotion eligibility, boundary bookkeeping, state, and proof; the heartbeat owns wakeup, role-context packet writing, mirror updates, Scott/Reid routing, and blocker reporting.
...
- Script-owned state/proof is written to `C:\Users\scott\Code\mindshare\roles\recruiter\level4-lifecycle-state.json` and `C:\Users\scott\Code\mindshare\roles\recruiter\level4-lifecycle-proof.md`.
...
- `C:\Users\scott\Code\mindshare\roles\recruiter\leadership-role-taxonomy.md` exists, is reviewed, and defines which leaders count as Level 5 and above for hiring-demand conversations.
- `C:\Users\scott\Code\mindshare\roles\recruiter\level5-company-position-research-policy.md` is reviewed and approved.
- `C:\Users\scott\Code\mindshare\roles\recruiter\level5-leader-hiring-demand-policy.md` is reviewed and approved.
...
| ana-script-timer-separation | Level 4 recurring lifecycle checks write state/proof or durable source | Ana script plus Tess/Reid routing | Block operational claim until script path, timer id, state/proof, and Reid route are recorded |
...
- Recurring Level 4 lifecycle logic exists only in a long heartbeat prompt when it can reasonably be implemented as a human-readable script.
...
Version: 1.13 (script-backed scoped lifecycle)
...
| 2026-06-24 | 1.13 | Converted Ana Level 4 lifecycle to script/timer methodology: `scripts\level4automation.py` owns deterministic backlog/timed-promotion eligibility, state, and proof while the heartbeat handles packet writing, mirrors, and routing. | Tess |
