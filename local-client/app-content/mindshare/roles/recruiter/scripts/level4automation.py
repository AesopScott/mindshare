...
What this script checks:
    ...
    - Pipeline items eligible for Level 2 -> Level 3.
    - The boundary that Ana's Level 4 stops at Level 3.
...
def build_state(mode: str) -> dict:
    ...
    for item in items:
        ...
        if status != "active" or eligible_at is None or eligible_at > now:
            continue
        if stage == "level_1_new_hire":
            l1_ready.append(item)
        elif stage == "level_2_trainee":
            l2_ready.append(item)
    result = "eligible_work" if backlog_eligible or l1_ready or l2_ready else "no_eligible_work"
    return {
        "schema_version": 1,
        "role": "Ana / Recruiter",
        ...
        "counts": {
            ...
            "level_2_ready_for_level_3": len(l2_ready),
        },
        ...
        "eligible_level_2_to_3": [
            {"id": item.get("id"), "role_title": item.get("role_title"), "next_eligible_at": item.get("next_eligible_at")}
            for item in l2_ready
        ],
        "gate_status": {
...
def proof_entry(state: dict) -> str:
    return "\n".join([
        ...
        "Denied actions held: no external recruiting, candidate contact, human hiring, office activation, Level 4+ promotion, Git/GitHub/release, production, website edit, spending, secrets, or authority expansion.",
        "",
    ])
