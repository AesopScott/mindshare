...
| M10 | `/multi-agent-deploy-observe` | Define runtime readiness: release authority, configuration, secrets, child APS deployment evidence, smoke checks, hold conditions, rollback, and M11 observation handoff. |
| M11 | `/multi-agent-improve` | Observe live system signals, classify evidence, incidents, traces, eval failures, user feedback, and operator findings, and route improvements across M2-M10 and child APS phases. |
