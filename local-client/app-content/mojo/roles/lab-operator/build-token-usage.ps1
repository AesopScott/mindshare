...
function Test-LooksLikePath([string]$value) {
...
if (Test-Path $sessionIndexPath) {
  foreach ($line in Get-Content -LiteralPath $sessionIndexPath) {
    ...
    if ([string]::IsNullOrWhiteSpace($entry.id) -or [string]::IsNullOrWhiteSpace($entry.thread_name)) {
      continue
    }

    try {
      $updatedAt = [datetime]$entry.updated_at
    } catch {
      $updatedAt = [datetime]::MinValue
    }

    if (-not $codexSessionTitles.ContainsKey($entry.id) -or $updatedAt -ge $codexSessionUpdated[$entry.id]) {
      $codexSessionTitles[$entry.id] = [string]$entry.thread_name
      $codexSessionUpdated[$entry.id] = $updatedAt
    }
  }
}

function Resolve-CodexTitle($row) {
  $rawTitle = [string]$row.room_title_or_cwd
  $sessionId = [string]$row.room_or_session

  if ($row.interface -ne 'Codex') {
    return $rawTitle
  }

  if ($sessionId -eq '(aggregate)') {
    return $rawTitle
  }

  if ($codexSessionTitles.ContainsKey($sessionId)) {
    return $codexSessionTitles[$sessionId]
  }

  if (-not [string]::IsNullOrWhiteSpace($rawTitle) -and -not (Test-LooksLikePath $rawTitle) -and $rawTitle -ne '(aggregate)') {
    return $rawTitle
  }

  return "Unresolved Codex title: $sessionId"
}
...
$codexRooms = $last24 |
  ...
  ForEach-Object {
    ...
    $looksLikePath = Test-LooksLikePath $rawTitle
    $title = Resolve-CodexTitle $latest
...
$largestCodexHtml = if ($largestCodex) {
  ($largestCodex | ForEach-Object {
    '<li><strong>' + (Fmt $_.total_tokens) + '</strong> - ' + (Escape-Html (Resolve-CodexTitle $_)) + ' <span class="muted">(' + (Escape-Html $_.room_or_session) + ')</span></li>'
  }) -join "`n"
} else {
