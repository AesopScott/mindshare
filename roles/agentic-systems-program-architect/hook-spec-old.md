...
Load Vik's architectural guidance when a session or request changes Mindshare/MAPS structure, roles, skills, memory, authority, or agent build paths.
...
- Before commit when staged files include Mindshare/MAPS structural artifacts.
...
2. Emit a short context packet pointing to `roles/agentic-systems-program-architect/role-agent.md`.
3. Never modify files directly.
4. Write memory only through the Mindshare foundation contract.
...
- Memory writes only through configured Mindshare stores.

## Failure Behavior

If the hook cannot determine scope, it should emit: "Vik review recommended: Mindshare architecture boundary is unclear." It should not block work unless Scott later approves a blocking gate.
