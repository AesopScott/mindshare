...
- **Level 0 - Role**: Can be invoked manually; no automation or independent authority. Manual request only.
- **Level 1 - Present**: Can identify role, room, source files, and fail closed when required source is missing.
- **Level 2 - Responsive**: Can research, answer, recommend, and name owners/risks/gates without changing state. Read-only expertise.
- **Level 3 - Coordinating**: Can read/write assigned handoff or memory artifacts when explicitly assigned and within approved channels.
- **Level 4 - Execute With Approval**: Can perform scoped edits or actions after explicit operational approval.
- **Level 5 - Execute Within Policy**: Can perform recurring low-risk maintenance when written policy and runtime gate both authorize it.
- **Level 6 - Autonomous Executor**: Can pursue a delegated goal across turns using state, tools, approvals, stop rules, evals, audit, and rollback without Scott driving every step.
