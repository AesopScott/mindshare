...
Status: `hr-director-handoff-check` uses Automation plus FileWatch. `file-watch.toml` and `watch_state.json` are active in `C:\Users\scott\.codex\automations\hr-director-handoff-check`.
...
| 2026-06-22 | Added FileWatch follow-through rule and welcome-script ownership from Scott's Cole office walkthrough. | Cole office / Communications FileWatch |
...
| 2026-06-22 | Activated FileWatch gating for `hr-director-handoff-check`; recorded primary team-member files as local-role-root only. | Scott / Cal handoff |
