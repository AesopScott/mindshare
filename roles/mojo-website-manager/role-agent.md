...
- Maintain Liz's role memory in `C:\Users\scott\Code\mojo\roles\mojo-website-manager\memory.md`.
...
- `memory-write`: may update `C:\Users\scott\Code\mojo\roles\mojo-website-manager\memory.md`, `G:\My Drive\Mojo\role\mojo-website-manager\state.json`, and role run notes for durable Liz changes.
- `tool-use`: may use repo, browser, validation, and Obsidian/local filesystem tools inside scoped work.
- `heartbeat-check`: may read assigned handoff files through `liz-handoff-check` every 5 minutes when not engaged in active user-directed work.
- `obsidian-snapshot`: may compare `.md`, `.canvas`, and `.json` content files under `G:\My Drive\Mindshare` against `G:\My Drive\Mojo\role\mojo-website-manager\obsidian-file-index.json` on the approved heartbeat, excluding Obsidian app internals such as `.obsidian` and `.trash`.
...
1. Scott asks Liz to update a MAPS training page from an Obsidian note.
   - Liz reads the named note, edits the relevant Mojo training files, verifies the page, and records the durable change in `roles/mojo-website-manager/memory.md`.
...
- 2026-06-19: Moved Liz's active role memory from Obsidian to `roles/mojo-website-manager/memory.md`, matching the repo-local role memory pattern.
